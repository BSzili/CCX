...
const char kEscapeKey = 0x1B; // ASCII code 27 as hex.
...
static MBoolean HiScoreSelected( int *item, unsigned char inKey, SDLKey inSDLKey )
{
	...
	switch (inSDLKey) {
		...
		case SDLK_BACKSPACE: // backspace key
			if( nameLength > 0 )
			{
//				printf("Should have trimmed name\n");
		...
		default:
			...
			{
//				printf("HiScoreSelected - key pressed\n");
...
//UNUSED	const MRect    fullRect = { 0, 0, 480, 640 };
	...
	while( ((dialogStage != kClosing) || !dialogStageComplete) && !finished )
	{
		...
		if( (dialogStage == kOpening) && dialogStageComplete )
		{
			...
			{
				/*NumDialogs enum looks like this.
				 kHiScoreDialog,
				 kContinueDialog,
				 kControlsDialog,
				 kNumDialogs*/
